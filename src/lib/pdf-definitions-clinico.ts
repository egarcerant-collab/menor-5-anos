...
function buildDocDefinition(data: InformeClinicoData, backgroundImageBase64: string): TDocumentDefinitions {
    const docDefinition: TDocumentDefinitions = {
        ...
        content: [
            ...
            { text: 'El presente Informe Técnico de Auditoría tiene como finalidad evaluar de manera integral la ejecución operativa, clínica y financiera del contrato N.° '+ data.contrato +', suscrito entre DUSAKAWI EPSI y la IPS '+ data.ips +', para la prestación de servicios de salud bajo la modalidad de Pago Global Prospectivo (PGP) en el municipio de Riohacha, departamento de La Guajira, durante el mes de '+ data.periodo +'.', style: 'p' },
            ...
            { text: `El contrato ${data.contrato} fue celebrado con la ${data.ips}, identificada con NIT ${data.nit}, con el objetivo de garantizar la atención integral de una población objetivo de ${data.poblacion.toLocaleString('es-CO')} afiliados residentes en el municipio de Riohacha.`, style: 'p' },
            ...
            { text: `La metodología aplicada para el presente informe se basó en un enfoque técnico, descriptivo y analítico, sustentado en la revisión de la matriz financiera y operativa correspondiente al mes de ${data.periodo}, suministrada por la ${data.ips}.`, style: 'p' },
            ...
            { text: `Durante el mes de ${data.periodo}, el contrato ${data.contrato} presentó un valor esperado de ${formatCOP(data.valorEsperado)}, estableciéndose un límite inferior de ${formatCOP(data.limiteInferior)} (-10%) y un límite superior de ${formatCOP(data.limiteSuperior)} (+10%), conforme a lo estipulado contractualmente.`, style: 'p' },
            { text: `El valor ejecutado para el periodo ascendió a ${formatCOP(data.valorEjecutado)}, lo que representa un nivel de ejecución del ${(data.valorEjecutado / data.valorEsperado * 100).toFixed(1)}% frente al valor programado. Desde el punto de vista estrictamente cuantitativo, esta cifra se ubica ligeramente por encima del límite superior, con una diferencia marginal que no configura un desbordamiento significativo del marco presupuestal.`, style: 'p' },
            ...
            { image: data.chartImage, width: 450, alignment: 'center', margin: [0, 5, 0, 10] },
            { text: 'El análisis financiero del contrato permite concluir que la ejecución registrada durante el mes de octubre se enmarca dentro de la lógica estructural del modelo PGP, el cual reconoce la necesidad de flexibilidad frente a variaciones normales de la demanda asistencial.', style: 'p' },
            { text: 'Si bien el valor ejecutado supera de manera marginal el límite superior del rango técnico, esta variación corresponde a un escenario previamente contemplado en el diseño contractual, y no a una falla en la planeación ni en los mecanismos de control financiero. La diferencia observada es mínima frente al total ejecutado y no compromete la sostenibilidad económica del contrato.', style: 'p' },
            { text: 'Desde una lectura gráfica, la ejecución se comporta como una curva controlada, ubicada en el extremo superior del rango esperado, sin evidenciar tendencias acumulativas que puedan generar riesgo financiero en el mediano plazo. Este comportamiento refleja una gestión financiera activa y adaptativa, coherente con las condiciones reales de prestación del servicio.', style: 'p' },

            { text: '7. HALLAZGOS RELEVANTES', style: 'h2', pageBreak: 'before' },
            {
                ul: [
                    'La ejecución financiera presenta una superación marginal del límite superior del rango técnico, sin impacto estructural sobre el contrato.',
                    'El comportamiento observado responde a la dinámica asistencial del territorio y no a deficiencias en la planeación financiera.',
                    'La lectura gráfica evidencia estabilidad y ausencia de tendencias de crecimiento descontrolado.',
                    'Se requiere mantener un seguimiento mensual riguroso para prevenir acumulaciones de sobre–ejecución en periodos posteriores.',
            ...
            { text: `El análisis integral de la ejecución del contrato ${data.contrato} en el municipio de Riohacha durante el mes de ${data.periodo} permite concluir que el comportamiento financiero observado es técnicamente controlado, contractualmente previsto y operativamente coherente con la dinámica del territorio.`, style: 'p' },
            { text: 'La leve superación del rango superior no configura un riesgo financiero relevante ni compromete la sostenibilidad del contrato, evidenciando una adecuada capacidad de respuesta del prestador y una correcta aplicación de los principios de flexibilidad del modelo PGP. El resultado refleja un equilibrio razonable entre planeación, ejecución y demanda asistencial.', style: 'p' },
...
export async function descargarInformeClinicoPDF(data: InformeClinicoData, backgroundImageBase64: string): Promise<void> {
    ...
    pdfMake.createPdf(docDefinition).download(`Informe_Clinico_${data.ips.replace(/\s/g, '_')}_${data.periodo}.pdf`);
}
